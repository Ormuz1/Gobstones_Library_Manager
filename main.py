...
def select_and_add_file_to_library():
    ...
    files = filedialog.askopenfilenames(
        filetypes=[('gbs files', "*.gbs")], multiple=True)
    were_changes_made = False
    for file in files:
        were_changes_made = were_changes_made or library_buffer.importFileToLibrary(
            file)
    window.destroy()
...
def select_and_save_library_to_file():
    ...
    path = filedialog.asksaveasfilename(
        filetypes=[("GBS file", ".gbs")], defaultextension=".gbs")
